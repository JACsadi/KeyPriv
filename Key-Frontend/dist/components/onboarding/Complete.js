...
const Complete = () => {
    ...
    return (<div className="w-full max-w-md p-6 bg-white rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="space-y-4">
        {email && (<div className="p-3 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-600">Email</p>
            <p className="font-medium">{email}</p>
          </div>)}
        {orgName && (<div className="p-3 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-600">Organization</p>
            <p className="font-medium">{orgName}</p>
          </div>)}
        {country && (<div className="p-3 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-600">Country</p>
            <p className="font-medium">{country}</p>
          </div>)}
        {subdomain && (<div className="p-3 bg-gray-50 rounded-lg">
      ...
      <div className="space-y-3 mt-8">
        <button className="w-full py-3 px-4 rounded-lg font-medium text-white bg-blue-600 hover:bg-blue-700" onClick={() => router.push('/onboarding/admin')}>
          Create Admin Account
        </button>
        <button className="w-full py-3 px-4 rounded-lg font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-300" onClick={() => router.push('/')}>
          Skip for Now
        </button>
      </div>
    </div>);
